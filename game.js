    ...
    for (let pos = 0; pos < allPositions.length; pos++) {
      ...
      if (targetItem) {
        ...
        if (cardFront) {
          ...
          // 位置に対応する正しいアルファベットを表記
          const alphabet = String.fromCharCode(65 + pos); // 位置0→A, 位置1→B, ..., 位置5→F
          cardFront.textContent = alphabet;
          console.log(`シャッフルカード: pos=${pos}, alphabet=${alphabet} (65+${pos}=${65+pos})`);
        }
        ...
        if (cardFront && !originalCard.classList.contains('matched')) {
          ...
          const alphabet = String.fromCharCode(65 + pos); // 位置0→A, 位置1→B, ..., 位置5→F
          cardFront.textContent = alphabet;
          console.log(`非シャッフルカード: pos=${pos}, alphabet=${alphabet} (65+${pos}=${65+pos})`);
        }
      }
    }
